...
class DrawingWrapper extends React.Component {
  constructor(props) {
    ...
    this.isDevice = (/android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i.test(navigator.userAgent.toLowerCase()))
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    ...
    if (newProps.activeDrawing !== this.props.activeDrawing) {
      ...
      if (newProps.activeDrawing === true && newProps.currentTab === this.props.screenIndex) {
        if (this.hasDrawing) {
    ...
    if (this.props.activeDrawing === true && this.props.currentTab === this.props.screenIndex) {
    ...
    // if (this.props.canvasContainerHeight != newProps.canvasContainerHeight) {
    //   this.setState({
    //     height: newProps.canvasContainerHeight
    //   })
    // }
  }
  ...
  orientationChange = () => {
    setTimeout(() => {
      if (this.props.activeDrawing === true && this.props.currentTab === this.props.screenIndex && this.props.widthChange.orientation === this.props.orientation && this.isDevice) {
        this.canvas.parentNode.style.display = 'block';
        if (this.state.width === "") {
          this.setDimensions();
        }
  ...
  setDimensions = () => {
    setTimeout(() => {
      ...
      console.log("parentDimension", parentDimension)
      this.setState({
  ...
  onMouseDown = (event) => {
    ...
    this.props.showDrawingOptionsHandler(false);
  }
  ...
  onMouseMove = (event) => {
    if (this.canvasMouseDown === true) {
      if (this.props.drawingToolOptions.tool === 'pencil') {
        ...
        this.eraseDrawing(event.offsetX, event.offsetY, this.props.drawingToolOptions.size * 3);
      }
    }
  }
  ...
  render() {
    ...
    // height = canvasContainerHeight && canvasContainerHeight !== null ? canvasContainerHeight : height;
    // height = height < "400" ? "400" : height;
    return (
      <canvas
        ref={(canvas) => { this.canvas = canvas; }}
        className={`canvas-style ${drawingToolOptions.tool === 'pencil' ? 'pencil-cursor ' + drawingToolOptions.color : drawingToolOptions.tool === 'eraser' ? 'eraser-cursor' : null}`}
        width={width}
        height={height}
      />
    )
